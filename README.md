...
### Create secret and link to default service account

    $ oc adm policy add-role-to-user view system:serviceaccount:rhsso-rhdg:default
...
### Verify server and CA cert in truststore

    $ keytool -list -v -keystore truststore.jks | grep -A 1 "Owner"

    $ openssl verify -CAfile xpaas.crt tls.crt
